...
class Group(Model):
    def __init__(self):
        ...
        self.relay_legs = 0
        self.competition_type: Optional[str] = None
    ...
    def update_data(self, data):
        ...
        self.competition_type = data.get('competition_type')
        if 'ranking' in data:
            if data['ranking']:
                self.ranking = Ranking()
...
class Result:
    def __init__(self):
        ...
        self.tourism_penalty_time: Optional[OTime] = None
        self.tourism_credit_time: Optional[OTime] = None
        self.tourism_penalty_points = 0
        self.tourism_stage_dsq_count = 0

        self._tourism_base_penalty_time: Optional[OTime] = None
        self._tourism_base_credit_time: Optional[OTime] = None
        self._tourism_base_scores = 0
        self._tourism_base_rogaine_score = 0
        self._tourism_base_scores_ardf = 0
    ...
    def to_dict(self):
        return {
            ...
            'tourism_penalty_time_msec': self.tourism_penalty_time.to_msec() if self.tourism_penalty_time else 0,
            'tourism_penalty_time': self.tourism_penalty_time.to_str() if self.tourism_penalty_time else '',
            'tourism_cutoff_time_msec': self.tourism_credit_time.to_msec() if self.tourism_credit_time else 0,
            'tourism_cutoff_time': self.tourism_credit_time.to_str() if self.tourism_credit_time else '',
            'tourism_penalty_points': self.tourism_penalty_points,
            'tourism_stage_dsq_count': self.tourism_stage_dsq_count,
    ...
    def get_tourism_info(self) -> str:
        ...
        parts = []
        tourism_penalty_time = getattr(self, 'tourism_penalty_time', None)
        tourism_credit_time = getattr(self, 'tourism_credit_time', None)
        tourism_penalty_points = getattr(self, 'tourism_penalty_points', 0)
        tourism_stage_dsq_count = getattr(self, 'tourism_stage_dsq_count', 0)

        if tourism_penalty_time and tourism_penalty_time.to_msec() > 0:
            parts.append(f"P+{tourism_penalty_time.to_str()}")
        if tourism_credit_time and tourism_credit_time.to_msec() > 0:
            parts.append(f"C-{tourism_credit_time.to_str()}")
        if tourism_penalty_points:
            parts.append(f"Pts-{tourism_penalty_points}")
        if tourism_stage_dsq_count:
            parts.append(f"DSQe:{tourism_stage_dsq_count}")
    ...
    def get_result(self) -> str:
        ...
        tourism_info = self.get_tourism_info()
        if tourism_info:
            ret += f" [{tourism_info}]"
        return ret
    ...
    def get_place(self):
        ...
        value = ''
        if self.place > 0:
            value = str(self.place)
        elif self.person and self.person.is_out_of_competition:
            value = translate('o/c')
...
class Race(Model):
    ...
    def update_data(self, dict_obj):
        ...
        if dict_obj['object'] == 'Race' and dict_obj['id'] == str(self.id):
            ...
            self.competition_type = dict_obj.get(
                'competition_type',
                CompetitionType.INDIVIDUAL.value,
            )
            self.tourism_judging_mode = dict_obj.get(
