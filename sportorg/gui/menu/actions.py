...
from sportorg.gui.dialogs.web_timing import WebTimingDialog
from sportorg.gui.dialogs.manual_finish_queue import ManualFinishQueueDialog
...
class ManualFinishAction(Action, metaclass=ActionFactory):
    def execute(self):
        ...
        dialog = getattr(self.app, 'manual_finish_queue_dialog', None)
        if dialog is None:
            dialog = ManualFinishQueueDialog(app=self.app)
            self.app.manual_finish_queue_dialog = dialog
...
class TourismStagesAction(Action, metaclass=ActionFactory):
    def execute(self):
        obj = race()
        ...
        TourismStagesDialog(self.app).exec()
        self.app.refresh()


class TourismPenaltiesAction(Action, metaclass=ActionFactory):
    def execute(self):
        obj = race()
        ensure_tourism_defaults(obj)
        ...
        TourismPenaltiesDialog(self.app).exec()
...
class WebTimingAction(Action, metaclass=ActionFactory):
    def execute(self):
        WebTimingDialog(self.app).exec()
